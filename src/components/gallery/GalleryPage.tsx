    ...
    <div className="flex flex-col h-full bg-surface-0 text-gray-200">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-divider shrink-0 flex-wrap">
        ...
        <select
          ...
          className="bg-surface-2 border border-divider text-gray-200 text-sm px-3 py-1.5 rounded-lg focus:outline-none max-w-[180px]"
        >
        ...
        <div className="flex rounded-lg overflow-hidden border border-divider">
          {(['all', 'shot', 'grid', 'asset'] as const).map(t => (
            <button
              ...
              className={`px-3 py-1.5 text-xs transition-colors ${filterType === t ? 'bg-brand-600 text-white' : 'bg-surface-2 text-gray-400 hover:text-gray-200'}`}
            >
            ...
            <div key={item.id} className="relative group aspect-square bg-surface-2 rounded-lg overflow-hidden">
              ...
              <div className="absolute bottom-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100">
                <a
                  ...
                  className="text-xs bg-surface-2/90 text-white px-1.5 py-0.5 rounded"
        ...
        {hasMore && (
          <div className="flex justify-center mt-4">
            <button
              ...
              disabled={loading}
              className="px-6 py-2 bg-surface-2 hover:bg-surface-3 text-gray-300 text-sm rounded-lg disabled:opacity-50"
            >
              {loading ? '加载中...' : '加载更多'}
            </button>
          </div>
