    ...
    <div ref={tableRef} className="overflow-auto">
      <table className="w-full text-xs border-collapse min-w-[900px]">
        <thead className="sticky top-0 z-10">
          <tr>
            <th className="text-gray-500 font-medium text-left px-2 py-1.5 border-b border-divider bg-surface-1 w-6">#</th>
            {headers.map((h, i) => (
              <th key={i} className="text-gray-400 font-medium text-left px-2 py-1.5 border-b border-divider bg-surface-1 whitespace-nowrap">
                {h}
              </th>
            ))}
            <th className="text-gray-500 font-medium text-left px-2 py-1.5 border-b border-divider bg-surface-1 w-16">操作</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className="group border-b border-divider hover:bg-surface-3/40 transition-colors">
              ...
              <td className="px-2 py-2">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    title="复制提示词"
                    className="p-1 text-gray-500 hover:text-indigo-400 rounded hover:bg-surface-3 transition-colors"
                  >
