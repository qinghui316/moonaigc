...
const CreatePage: React.FC<{ loadedRecord?: HistoryRecord | null; loadedEpisode?: Episode | null }> = ({ loadedRecord, loadedEpisode }) => {
  ...
  const doGenerate = useCallback(async (cleanPlot: string) => {
    ...
    try {
      if (mode === 'single') {
        const systemPrompt = buildGenerateSystemPrompt(buildSystemContext())
              ...
              <div className="flex gap-1.5">
                <button onClick={() => setMode('chain')}
                  className={`flex-1 py-1.5 text-xs rounded-lg border font-medium transition-colors ${
                    mode === 'chain' ? 'border-amber-500 bg-amber-900/30 text-amber-300' : 'border-gray-700 text-gray-400'
                  }`}>
                  ⛓️ 链式引擎
                </button>
