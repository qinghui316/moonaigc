    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-surface-1 border border-divider-strong rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-divider">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-white w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-3 transition-colors">✕</button>
        </div>

        {/* Tabs */}
        <div className="flex bg-surface-2 mx-4 mt-4 rounded-lg p-1 gap-1">
          {(['text', 'vision', 'image'] as const).map(t => (
            <button key={t} onClick={() => { setActiveTab(t); setTestResult(null) }}
              className={`flex-1 py-1.5 text-sm font-medium rounded-md transition-colors ${
                activeTab === t ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white'
              }`}>
        ...
        <div className="p-4 space-y-4">
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    {'badge' in platform && platform.badge && (
                      <span className="ml-auto text-xs bg-brand-600 text-white px-1 rounded">
                        {platform.badge}
                      </span>
                    )}
                  </div>
          ...
          <div>
            ...
            {selectedPlatform.endpoint ? (
              <div className="w-full bg-surface-2/50 border border-divider/50 text-gray-400 text-sm px-3 py-2 rounded-lg select-all truncate">
                {selectedPlatform.endpoint}
              </div>
            ) : (
              <input
                ...
                className="w-full bg-surface-2 border border-divider text-gray-200 text-sm px-3 py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-500/40 focus:border-brand-500/60"
          ...
          <div>
            ...
            {(!selectedPlatform.models.some(m => m.value === settings.model) || settings.model === '') && (
              <input
                ...
                className="w-full mt-1.5 bg-surface-2 border border-divider text-gray-200 text-sm px-3 py-2 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-500/40 focus:border-brand-500/60"
              ...
              {[
                ...
                <label key={key} className="flex items-center justify-between cursor-pointer">
                  ...
                  <div
                    ...
                    className={`w-10 h-5 rounded-full transition-colors cursor-pointer relative ${val ? 'bg-indigo-500' : 'bg-surface-3'}`}
                  >
