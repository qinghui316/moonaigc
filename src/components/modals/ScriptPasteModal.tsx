    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface-1 rounded-xl w-full max-w-2xl shadow-2xl border border-divider-strong max-h-[90vh] flex flex-col"
        ...
        <div className="flex items-center justify-between p-5 border-b border-divider shrink-0">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white hover:bg-surface-3 rounded-lg">
        ...
        <div className="p-5 flex-1 overflow-auto">
          <textarea
            ...
            className="w-full h-64 bg-surface-2 border border-divider text-sm text-gray-200 placeholder-gray-600 px-3 py-2.5 rounded-lg focus:outline-none focus:ring-1 focus:ring-brand-500/40 focus:border-brand-500/60 resize-none"
          />
          <p className="text-xs text-gray-600 mt-2">{text.length} 字</p>
        </div>

        {/* Footer */}
        <div className="p-5 border-t border-divider flex justify-end gap-3 shrink-0">
          <button onClick={onClose} className="px-4 py-2 text-sm text-gray-400 hover:text-white border border-divider hover:border-gray-500 rounded-lg transition-colors">
            取消
          </button>
          <button
